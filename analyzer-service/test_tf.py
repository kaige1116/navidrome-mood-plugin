...
script = """apt-get update && apt-get install -y build-essential git libyaml-dev libfftw3-dev libavcodec-dev libavformat-dev libavutil-dev libswresample-dev libsamplerate0-dev libtag1-dev libchromaprint-dev libeigen3-dev pkg-config python3-dev
...
with open("analyzer-service/test.sh", "wb") as f:
    f.write(script.encode('utf-8'))

p = subprocess.Popen(['docker', 'run', '--rm', '-v', 'C:/Development/navidrome-mood-plugin-master/analyzer-service/test.sh:/test.sh', 'python:3.11-slim', 'bash', '/test.sh'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
